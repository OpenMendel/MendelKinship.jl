...
export standarize!, sample_mean_std, print_sample_stats, sample_stats
export simes_fdr, regress, glm_score_test, newton
...
function regress(X::Matrix{Float64}, y::Vector{Float64}, model::AbstractString)
  ...
  for iteration = 1:10
    ...
    if model == "logistic"
      ...
      score = BLAS.gemv('T', -1.0, X, z) # score = - X' * (z - y)
      w = sqrt.(w)
      LinearAlgebra.scale!(w, X) # diag(w) * X
      information = BLAS.gemm('T', 'N', X, X) # information = X' * W * X
      w = 1.0 ./ w
      LinearAlgebra.scale!(w, X)
    elseif model == "Poisson"
      z = exp.(z)
      w = copy(z)
      BLAS.axpy!(-1.0, y, z) # z = z - y
